...
# RelationScoreModule
class RelationScoreModule(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        super(RelationScoreModule, self).__init__()
        self.mlp = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, 1)  # Output a single score for each view
        )
        self.sigmoid = nn.Sigmoid()

    def forward(self, image_features, text_features):
        # image_features: [batch_size, N, C]
        # text_features: [batch_size, T, C]

        # We need to reshape and broadcast text_features to match the image features
        # image_features: [batch_size, N, C] --> [batch_size, N, C]
        # text_features: [batch_size, T, C] --> [batch_size, T, C]

        # batch_size, N, C = image_features.size()
        # B, T, N_ = text_features.size()

        assert image_features.shape(-1) == text_features.shape(-1), "dim of views/features should match."

        # Compute relation score for each (image, text) pair
        relation_scores = []
        for t in range(T):
            text_view = text_features[:, t, :]  # [batch_size, N]
            scores = self.mlp(torch.cat([image_features, text_view.unsqueeze(1)],
                                        dim=-1))  # [batch_size, N, 1]
            relation_scores.append(scores)

        # Stack relation scores and apply sigmoid
        relation_scores = torch.stack(relation_scores, dim=1)  # [batch_size, T, N]
        relation_scores = self.sigmoid(relation_scores)  # [batch_size, T, N]
...
class FusionNetwork(nn.Module):
    def __init__(self, image_dim, text_dim, hidden_dim):
        ...
        self.relation_score_module = RelationScoreModule(image_dim + text_dim, hidden_dim)

    def forward(self, image_features, text_features):
        # image_features: [batch_size, N, C]
        # text_features: [batch_size, T, N]
        relation_scores = self.relation_score_module(image_features, text_features)  # [batch_size, T, N]
        # Enhance image features using relation scores (residual connection)

        # For each view, multiply by relation score (1 + RSi(P,V))
        enhanced_features = image_features * (1 + relation_scores.sum(dim=1).unsqueeze(-1))  # [batch_size, N, C]

        return enhanced_features  # [batch_size, N, C]


@registry.register_model("blip2_vicuna_instruct_prompt_v4_malmm")
class Blip2VicunaInstruct_MALMM(Blip2Base):
    ...
    def __init__(
        ...
        self.FusionNetwork = FusionNetwork(
            image_dim=self.visual_hidden_size,
            text_dim=self.bert_hidden_size,
            hidden_dim=max(self.bert_hidden_size, self.visual_hidden_size) * 2
        )
        ...
        if self.qformer_text_input:  # 检查是否用QFormer处理文本输入
            ...
            if self.use_memory_bank:
                ...
                for t in range(T):
                    ...
                    # 让 prompt 与 visual encoder 进行 Relation Score
                    if self.linear_text_to_visual is not None:
                        prompt_embeds = self.linear_text_to_visual(prompt_embeds)  # [B, T, C]

                    self.FusionNetwork = self.FusionNetwork.to(image.device)  # 这个to(device)应该可以放在下面
                    prompt_visual_cross = self.FusionNetwork(
