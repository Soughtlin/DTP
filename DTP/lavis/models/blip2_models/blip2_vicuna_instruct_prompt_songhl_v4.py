    ...
    def forward(self, samples):
        ...
        if self.qformer_text_input:  # 检查是否用QFormer处理文本输入
            ...
            if self.use_memory_bank:
                ...
                text_Qformer_output = self.tokenizer(
                    samples["text_output"],  # [B]
                    padding='longest',
                    truncation=True,
                    max_length=self.max_txt_len,
                    return_tensors="pt",
                ...
                # 测试input和output
                # print("input: ", text_Qformer.input_ids)
                # print("output: ", text_Qformer_output.input_ids)
                # print("text input size: ", text_Qformer.input_ids.size())
                # print("text output size: ", text_Qformer_output.input_ids.size())
                ...
                for t in range(T):
                    ...
                    # 给question和answer都编码，分别作为K和V
                    prompt_embeds = self.Qformer.bert.embeddings(text_Qformer.input_ids)  # [B, T1, D1]
                    answer_embeds = self.Qformer.bert.embeddings(text_Qformer_output.input_ids)  # [B, T2, D2]

                    # 这里线型层采用了共享的，后面可以尝试用不共享的
                    if self.linear_text_to_visual is not None:
                        answer_embeds = self.linear_text_to_visual(answer_embeds)  # [B, T1, C]
                        prompt_embeds = self.linear_text_to_visual(prompt_embeds)  # [B, T2, C]
                    # print("mapped embeds input size: ", prompt_embeds.size())
                    # print("mapped embeds output size: ", answer_embeds.size())

                    # 连接prompt和answer
                    prompt_answer_embeds = torch.cat([prompt_embeds, answer_embeds], dim=1)

                    # print("padded embeds input size: ", prompt_embeds.size())
                    # print("padded embeds output size: ", answer_embeds.size())

                    # 使用 Cross Attention，将 image_embeds 作为 Q， answer_embeds作为K, prompt_embeds作为V
                    # K和V可以互换一下尝试是否效果更好
                    ...
                    prompt_visual_cross = self.cross_attention(
                        ...
                        key=prompt_answer_embeds,  # [B, T_text, C]
                        value=prompt_answer_embeds,  # [B, T_text, C]
                        attention_mask=None
                    )  # 输出[B, N, C]
