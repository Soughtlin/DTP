...
@registry.register_model("blip2_vicuna_instruct_apt_v0_malmm")
class Blip2VicunaInstruct_MALMM(Blip2Base):
    ...
    def __init__(
        ...
        self.atp_selector = ATPSelectorModel(self.atp_config)  # 将 ATP 模型放到设备上
    ...
    def forward(self, samples):
        ...
        if self.qformer_text_input:  # 检查是否用QFormer处理文本输入
            ...
            if self.use_memory_bank:
                ...
                # 给prompt使用bert编码 -> 作为ATP的x_txt_query #[B, sequence_length, D1]
                # # 使用[CLS]或均值池化的方式，将序列化的文本嵌入压缩为单个全局向量（形状为(N, D_in)）
                # prompt_embeds = self.Qformer.bert.embeddings(text_Qformer.input_ids).to(
                #     image.device)  # [B, sequence_length, D1]
                # # 方案1:[CLS]
                # prompt_cls_embeds = prompt_embeds[:, 0, :]  # [B, D1]
                # # 方案2:均值池化

                selected_T = 0  # 用于记录新帧序列的总帧数
                selected_frames_list = []  # 用于存储每次循环的 selected_frames
                # Process every 10 frames at a time
                frame_step = 2
                for t in range(0, 4, frame_step):
                    end_idx = min(t + frame_step, T - 1)  # Ensure the last segment is properly bounded
                    frames = image[:, :, t:end_idx, :, :]  # (B, C, frame_step, H, W)
                    ...
                    with torch.no_grad():  # [B,frame_step, D]
                        ...
                        # atp进行帧选择
                    # print(f"frames feature: {frames_feature.shape}")
                    # print(f"prompt cls feature: {prompt_cls_embeds.shape}")
                    selected_frame_feature, selection_mask = self.atp_selector(
                        x_vis_seq=frames_feature,
                        x_txt_query=prompt_feature,
                        x_txt_cands=None
                    )
                    # 通过mask计算出选择出来的帧在原输入的帧序列中的index
                    # selection_index = get_selected_index_from_selection_mask(
                    #     frame_idxs_gt=frames_feature,
                    #     selection_mask=selection_mask,
                    #     sequence_first=False
                    # )
                    # print(f"selection_index: {selection_index}")
                    # 利用高级索引从原来帧序列中抽出对应的帧
                    # batch_indices = torch.arange(frames.size(0), device=frames.device)  # [0, 1, ..., B-1]
                    # selected_frames = frames[batch_indices, :, selection_index, :, :]  # [B, C, H, W]
                    ...
                    selected_T += 1  # 每次循环增加的帧数
                    selected_frames_list.append(selected_frames)  # 将当前帧添加到列表中

                # 将所有 selected_frames 拼接成新的帧序列
                selected_frames_seq = torch.cat([selected_frames_list, image[:, :, 10:T, :, :]],
                                                dim=2)  # 拼接，形状: [B, C, T1, H, W]
                selected_T += T - 4
                # print(f"新帧序列的形状: {selected_frames_seq.shape}")
                # print(f"总帧数 T1: {selected_T}")
                # print(f"ATP筛选帧数: {selected_T}")
                # print(f"selected_frames_seq{selected_frames_seq}")
                # 将筛选后的帧作为输入
                for t in range(0, selected_T, 1):
                    with self.maybe_autocast():
                        ...
                        image_embeds = self.ln_vision(
                            self.visual_encoder(selected_frames_seq[:, :, t, :, :]))  # [B, 256+1, 1408]
