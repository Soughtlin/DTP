...
from lavis.models.blip2_models.atp import ATPSelectorModel, ATPConfig, get_selected_index_from_selection_mask, \
    select_frame_from_mask
from lavis.models.blip2_models.atp_utils import VideoFeatureExtractor
...
class frame_selector:
    def __init__(self, atp_config):
        ...
        # 初始化 ATP 模型
        self.atp_config = atp_config
        self.atp_selector = ATPSelectorModel(self.atp_config)

    def select_frames(self, image, text_input, start_idx=0, frame_step=5, train=True, device="cuda"):
        """
        对指定范围内的帧进行帧选择并覆盖原序列。
        :param image: 输入的视频帧张量 (B, C, T, H, W)
        :param text_input: 包含文本输入的字典
        :param start_idx: 需要选择的起始帧索引
        :param frame_step: 需要处理的帧数（从 start_idx 开始的数量）
        """
        image = image.to(device)
        self.feature_extractor.to(device)
        self.atp_selector.to(device)
        T = image.size(2)

        # 计算结束帧索引
        end_idx = min(start_idx + frame_step, T - 1)

        # 确保范围合理
        if start_idx < 0 or frame_step <= 0:
            raise ValueError("Invalid parameters: start_idx must be >= 0 and frame_step must be > 0")

        # 获取指定帧范围
        frames = image[:, :, start_idx:end_idx, :, :]  # (B, C, frame_count, H, W)

        # 使用 atp_selector 选择帧
        if train:
            with torch.no_grad():
                frames_feature, prompt_feature = self.feature_extractor(
                    video_frames=frames,
                    text_inputs=text_input
                )
            frames_feature.to(device)
            prompt_feature.to(device)
            selected_frame_feature, selection_mask = self.atp_selector(
                x_vis_seq=frames_feature,
                x_txt_query=prompt_feature,
                x_txt_cands=None
            )
        else:
            ...
            frames_feature.to(device)
            prompt_feature.to(device)
            self.atp_selector.eval()
            selected_frame_feature, selection_mask, _logits = self.atp_selector(
                x_vis_seq=frames_feature,
                x_txt_query=prompt_feature,
                x_txt_cands=None
            )

        frame_idx = get_selected_index_from_selection_mask(
            frame_idxs_gt=frames,
            selection_mask=selection_mask
        )

        # 根据 mask 筛选出帧
        selected_frames = select_frame_from_mask(
            frames=frames,
            selection_mask=selection_mask
        )  # [B, C, H, W]
        ...
        # # 覆盖原视频序列指定范围的帧
        # if start_idx == 0:
        #     new_image = torch.cat([selected_frames, image[:, :, end_idx:, :, :]], dim=2)
        # elif end_idx == T:
        #     new_image = torch.cat([image[:, :, :start_idx, :, :], selected_frames], dim=2)
        # else:
        #     new_image = torch.cat(
        #         [image[:, :, :start_idx, :, :], selected_frames, image[:, :, end_idx:, :, :]], dim=2)
        # print(f"new image.shape: {new_image.shape}")

        return frame_idx, selected_frames


@registry.register_model("blip2_vicuna_instruct_apt_v2_malmm")
class Blip2VicunaInstruct_MALMM(Blip2Base):
    ...
    def __init__(
        ...
        self.Qformer, self.query_tokens = self.init_Qformer(
            ...
            # num_frames=math.ceil(num_frames / 10),
        )
        ...
        self.frame_selector = frame_selector(self.atp_config)
    ...
    def forward(self, samples):
        ...
        if self.qformer_text_input:  # 检查是否用QFormer处理文本输入
            if is_video:
                ...
                text_input = [text for text in samples["text_input"] for _ in range(T - 4)]
            else:
            ...
            if self.use_memory_bank:
                ...
                # 仅处理前5帧
                with torch.no_grad():
                    select_idx, select_frame = self.frame_selector.select_frames(
                        image=image,
                        text_input=samples["text_input"],
                        start_idx=0,
                        frame_step=5,
                ...
                # 创建 select_idx_list，包含 select_idx 和 [5, T] 之间的所有帧
                select_idx_list = list(range(5, T)) + [select_idx]  # [5, T] 与 select_idx 合并成一个列表
                ...
                for t in range(T):
                    with self.maybe_autocast():
                        ...
                        # 判断当前帧是否在 select_idx_list 中
                        if t in select_idx_list:
                            # 如果是被选中的帧，正常计算其 image_embeds
                            image_embeds = self.ln_vision(self.visual_encoder(image[:, :, t, :, :]))  # [B, 256+1, 1408]
                        else:
                            # 如果是未被选中的帧，设置 image_embeds 为零
                            image_embeds = torch.zeros_like(
                                self.ln_vision(self.visual_encoder(image[:, :, t, :, :])))  # [B, 256+1, 1408]
    ...
    def generate(
        ...
        if self.qformer_text_input:
            ...
            if self.use_memory_bank:
                ...
                # start_idx = 0
                # frame_step = 5
                # new_image, new_T = self.frame_selector.select_frames(
                #     image=image,
                #     text_input=prompt,
                #     start_idx=start_idx,
                ...
                for t in range(T):
                    ...
                    # print(f"memory bank: {self.visual_memory_bank.shape}")
                    query_output = self.Qformer.bert(
                        text_Qformer.input_ids,
                        attention_mask=Qformer_atts,
                        query_embeds=query_tokens,
