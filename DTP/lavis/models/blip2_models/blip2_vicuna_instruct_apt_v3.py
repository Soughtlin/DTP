...
class frame_selector:
    ...
    def select_frames(self, image, text_input, start_idx=0, frame_step=5, train=True, device="cuda"):
        ...
        # 计算结束帧索引
        end_idx = min(start_idx + frame_step, T)
        ...
        new_T = T - frame_step + 1
        return new_image, new_T


@registry.register_model("blip2_vicuna_instruct_apt_v3_malmm")
                ...
                for t in range(new_T):
                    with self.maybe_autocast():
                        ...
                        image_embeds = self.ln_vision(
                            self.visual_encoder(new_image[:, :, t, :, :]))  # [B, 256+1, 1408]
        ...
        if self.qformer_text_input:
            ...
            if self.use_memory_bank:
                ...
                for t in range(new_T):
                    with self.maybe_autocast():
                        image_embeds = self.ln_vision(
                            self.visual_encoder(new_image[:, :, t, :, :])).detach()  # [B, 256+1, 1408]
                    N, C = image_embeds.shape[-2:]
                    position_ids = torch.tensor([t]).long().to(image_embeds.device)  # [1]
                    position_ids = position_ids.unsqueeze(0).expand(B, -1)  # [B, 1]
