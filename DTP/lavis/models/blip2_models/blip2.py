    ...
    def forward(
        ...
        if is_cross_attention:
            ...
            if hasattr(self, 'query_memory_bank'):
                ...
                if k.size(0) != self.key(query_memory_bank).size(0):
                    print(f"hidden states: {hidden_states.shape}")
                    print(f"k: {k.shape}")
                    print(f"query_memory_bank: {query_memory_bank.shape}")
                    print(f"key(query_memory_bank): {self.key(query_memory_bank).shape}")
                    if self.key(query_memory_bank).size(0) > k.size(0):
                        padding = torch.zeros(self.key(query_memory_bank).size(0) - k.size(0), k.size(1), k.size(2), device=k.device)
                        k = torch.cat([k, padding], dim=0)  # 填充k
                    else:
