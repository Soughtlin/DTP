...
class CrossAttention(nn.Module):
    ...
    def forward(self, query, key, value, attention_mask=None):
        # query, key, value 的形状分别是 [B, 1, N, C] 和 [B, 1, T, C]
        B, _, N, _ = query.size()  # N 是 query 的长度
        _, _, T, _ = key.size()  # T 是 key 和 value 的长度

        # 线性变换
        Q = self.query_fc(query)  # [B, 1, N, hidden_size]
        K = self.key_fc(key)  # [B, 1, T, hidden_size]
        V = self.value_fc(value)  # [B, 1, T, hidden_size]

        # 拆分为多头 [B, 1, N, num_heads, head_dim] 这样每个头有一个不同的维度
        assert self.hidden_size % self.num_heads == 0, "hidden_size 必须能被 num_heads 整除"
        head_dim = self.hidden_size // self.num_heads

        # 将 Q, K, V 重塑为多头形式
        Q = Q.view(B, 1, N, self.num_heads, head_dim).transpose(2, 3)  # [B, 1, num_heads, N, head_dim]
        K = K.view(B, 1, T, self.num_heads, head_dim).transpose(2, 3)  # [B, 1, num_heads, T, head_dim]
        V = V.view(B, 1, T, self.num_heads, head_dim).transpose(2, 3)  # [B, 1, num_heads, T, head_dim]

        # 计算点积注意力
        attention_scores = torch.matmul(Q, K.transpose(-2, -1))  # [B, 1, num_heads, N, T]
        ...
        # Softmax 和 Dropout
        attention_weights = F.softmax(attention_scores, dim=-1)  # [B, 1, num_heads, N, T]
        attention_weights = self.dropout(attention_weights)

        # 加权求和得到输出
        attention_output = torch.matmul(attention_weights, V)  # [B, 1, num_heads, N, head_dim]

        # 合并多头输出
        attention_output = attention_output.transpose(2, 3).contiguous().view(B, 1, N, self.hidden_size)

        # 通过线性层得到最终输出
        attention_output = self.out_fc(attention_output)  # [B, 1, N, embed_size]

        return attention_output


@registry.register_model("blip2_vicuna_instruct_prompt_v0_malmm")
        ...
        if self.qformer_text_input:  # 检查是否用QFormer处理文本输入
            ...
            if self.use_memory_bank:
                ...
                for t in range(T):
                    ...
                    # batchsize能保持一致吗？ 可以
                    # 方案1
                    prompt_embeds = self.Qformer.bert.embeddings(text_Qformer.input_ids)  # [B, T, D]
                    # 方案2
                    # prompt_embeds = query_output.last_hidden_state  # [B, T, D]

                    prompt_embeds = prompt_embeds.unsqueeze(1)  # [B,1,T,D]

                    # !!! 将cross_attention放在bert前面，其输出
                    # 让 prompt 与 visual encoder 进行cross attention
                    if self.linear_text_to_visual is not None:
                        prompt_embeds = self.linear_text_to_visual(prompt_embeds)  # [B,1, T, C]
                    ...
                    prompt_visual_cross = self.cross_attention(
                        query=image_embeds,  # [B, 1,N, C]
                        key=prompt_embeds,  # [B, 1,T_text, C]
                    ...
                    prompt_visual_cross = image_embeds + prompt_visual_cross  # [B,1, N, C]
            ...
            if self.use_memory_bank:
                ...
                for t in range(T):
                    ...
                    prompt_visual_cross = self.cross_attention(
                        query=image_embeds,  # [B,1, N, C]
                        key=prompt_embeds,  # [B,1, T_text, C]
                        value=prompt_embeds,  # [B,1, T_text, C]
                        attention_mask=None
                    )  # 输出[B,1, N, C]
