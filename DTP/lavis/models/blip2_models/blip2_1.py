...
from lavis.models.blip2_models.Qformer import BertConfig, BertLMHeadModel, BertSelfAttention, BertAttention, BertLayer, \
    ...
    def forward(
        ...
        if is_cross_attention:
            ...
            if hasattr(self, 'query_memory_bank'):
                ...
                if hidden_states.size(0) != query_memory_bank.size(0):
                    print(f"hidden states: {hidden_states.shape}")
                    print(f"query_memory_bank: {query_memory_bank.shape}")
                    if query_memory_bank.size(0) > hidden_states.size(0):
                        print("Batch size mismatch: The batch size of hidden_states < query_memory_bank")
                        padding = torch.zeros(query_memory_bank.size(0) - hidden_states.size(0), hidden_states.size(1),
                                              hidden_states.size(2), device=hidden_states.device)
                        print(f"padding: {padding.shape}")
                        hidden_states = torch.cat([hidden_states, padding], dim=0)  # 填充k
                        print(f"padded k: {hidden_states.shape}")
                    else:
                        raise ValueError("Batch size mismatch: The batch size of k < key(query_memory_bank) ")
        ...
        if attention_mask is not None and not is_cross_attention:
            ...
            if hasattr(self, 'query_memory_bank'):
                attention_scores = attention_scores + torch.cat([attention_mask] * (self.query_memory_bank.size(1) + 1),
                                                                dim=-1)
            else:
        ...
        if not is_cross_attention:
            ...
            if not hasattr(self, 'query_memory_bank'):
                ...
                self.query_memory_bank = torch.cat([self.query_memory_bank, hidden_states[:, None, :, :].detach()],
                                                   dim=1)  # [B, t+1, 32, C]
            ...
            if self.compression_size.sum(1).mean().round() == self.num_frames:
                ...
                self.query_memory_bank, self.compression_size = memory_bank_compress(self.query_memory_bank,
...
class Blip2Base(BaseModel):
    ...
    def init_tokenizer(cls, truncation_side="right"):
        tokenizer = BertTokenizer.from_pretrained("/lrh/MA-LMM-Songhl/MA-LMM-Songhl/bert-base-uncased",
    ...
    def init_Qformer(cls, num_query_token, vision_width, cross_attention_freq=2, memory_bank_length=0, num_frames=0):
        encoder_config = BertConfig.from_pretrained("/lrh/MA-LMM-Songhl/MA-LMM-Songhl/bert-base-uncased")
        ...
        Qformer = BertLMHeadModel.from_pretrained(
            "/lrh/MA-LMM-Songhl/MA-LMM-Songhl/bert-base-uncased", config=encoder_config
        )
...
def memory_bank_compress(memory_bank: torch.Tensor, compression_size: torch.Tensor) -> tuple:
    ...
    src_memory_bank = (src_memory_bank * src_size.unsqueeze(-1)).clone()
    # dst_memory_bank *= dst_size.unsqueeze(-1)
    dst_memory_bank = (dst_memory_bank * dst_size.unsqueeze(-1)).clone()

    # Compress the memory bank by adding the source memory bank to the dst memory bank
    dst_memory_bank.scatter_add_(dim=1, index=max_similarity_indices.unsqueeze(-1).expand(-1, -1, -1, C),
                                 src=src_memory_bank)
