...
# 这版将输入T的前一半用image，后一半用cross

# 测试完v2再测试这个，如果v2的效果好，cross attention就用v2的，否则还用v1的
...
@registry.register_model("blip2_vicuna_instruct_prompt_v3_abl3_malmm")
    ...
    def forward(self, samples):
        ...
        if self.qformer_text_input:  # 检查是否用QFormer处理文本输入
            ...
            if self.use_memory_bank:
                ...
                # print("query_atts: ", query_atts.size())
                # 将查询 token和文本token的mask拼接，使得整个Q-Former可以同时处理查询和文本的注意力交互
                Qformer_atts = torch.cat([query_atts, text_Qformer.attention_mask], dim=1)
                # print("Qformer_atts: ", Qformer_atts.size())
                # 逐帧遍历
                ...
                for t in range(T):
                    ...
                    # print("image_atts", image_atts.size())
                    # 给image扩展出时间维度
                    image_embeds = image_embeds.unsqueeze(1)  # [B, 1, N, C]

                    # 用于更新visual_memory_bank
                    visual_memory_bank_input = image_embeds

                    # 前1/8的帧继续使用原vusial encoder来更新visual memory bank, 后面的帧再使用cross attention
                    if t > 3:
                        ...
                        visual_memory_bank_input = prompt_visual_cross

                    # 输入visual memory bank
                    # 如果是第一帧，初始化visual_memory_bank作为第一帧的嵌入
                    if t == 0:
                        # !!! encoder_hidden_states 直接使用ross_attention的输出进行初始化
                        encoder_hidden_states = visual_memory_bank_input  # [B, 1, N, C] encoder_hidden_states表示模型当前对视觉特征的理解
                        # compress(不可微?)之前要把梯度断掉，detach不能随便动
                        # 学习.detach() .clone()  .cpu()v .numpy()
                        self.visual_memory_bank = visual_memory_bank_input.detach()  # [B, 1, N, C]
                        ...
                        encoder_hidden_states = torch.cat([self.visual_memory_bank, visual_memory_bank_input],
                                                          dim=1)  # [B, (t+1), N, C]
                        # 将visual_memory_bank与当前帧嵌入连接，并更新compression_size
                        self.visual_memory_bank = torch.cat(
                            [self.visual_memory_bank, visual_memory_bank_input.detach()],
                            dim=1)  # [B, t+1, N, C]
        ...
        if self.qformer_text_input:
            ...
            if self.use_memory_bank:
                ...
                for t in range(T):
                    ...
                    image_embeds_ori = image_embeds  # 用于后面cross attention
                    image_embeds = image_embeds.unsqueeze(1)  # [B, 1, N, C]
                    image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)  # [B, N]
                    # 用于更新visual_memory_bank
                    visual_memory_bank_input = image_embeds

                    # 前一半的帧继续使用原vusial encoder来更新visual memory bank, 后一半的帧再使用cross attention
                    if t > 10:
                        # 给prompt使用bert编码 [batchsize, sequence_length, C]
                        # batchsize能保持一致吗？ 可以
                        # 方案1
                        ...
                        # 用torch提供的多头注意力
                        # prompt_visual_cross = self.nn_cross_attention(
                        #     image_embeds_ori,  # [B, N, C]
                        #     prompt_embeds,  # [B, T_text, C]
                        #     prompt_embeds,  # [B, T_text, C]
                        # )  # 输出[B, N, C]
                    ...
                    if t == 0:
                        self.visual_memory_bank = visual_memory_bank_input  # [B, 1, N, C]
                        self.size_constant = torch.ones(B, 1, N).to(image_embeds.device)  # [B, 1, N]
                        self.compression_size = self.size_constant  # [B, 1, N]
                    else:
                        self.visual_memory_bank = torch.cat([self.visual_memory_bank, visual_memory_bank_input],
                                                            dim=1)  # [B, t+1, N, C]
                        self.compression_size = torch.cat([self.compression_size, self.size_constant],
                                                          dim=1)  # [B, t+1, N]
