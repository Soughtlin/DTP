...
class CrossAttention(nn.Module):
    ...
    def forward(self, query, key, value, attention_mask=None):
        ...
        Q = Q.view(B, T_q, self.num_heads, self.embed_size // self.num_heads).transpose(1, 2)  # [B,h,T_q,D]
        K = K.view(B, T_k, self.num_heads, self.embed_size // self.num_heads).transpose(1, 2)  # [B,h,T_k,D]
        V = V.view(B, T_k, self.num_heads, self.embed_size // self.num_heads).transpose(1, 2)  # [B,h,T_k,D]
        # 计算点积注意力
        attention_scores = torch.matmul(Q, K.transpose(-2, -1))  # [B,h,T_q,D] * [B,h,D,T_k] -> Q[B, h, T_q, T_k]
        attention_scores = attention_scores / (self.embed_size // self.num_heads) ** 0.5  # 缩放
        ...
        # 最终输出
        output = self.out_fc(attention_output)  # [B, T_q, embed_size]
        return output
        ...
        if self.qformer_text_input:  # 检查是否用QFormer处理文本输入
            ...
            if self.use_memory_bank:
                ...
                for t in range(T):
                    ...
                    image_embeds_ori = image_embeds  # [B,N,C]保留原始的image_embeds用于后面cross attention
                    ...
                    # print("Query shape:", image_embeds_ori.shape)
                    # print("Key shape:", prompt_embeds.shape)
                    # print("Value shape:", prompt_embeds.shape)
        ...
        if self.qformer_text_input:
            ...
            if self.use_memory_bank:
                ...
                for t in range(T):
                    ...
                    # 待定：推理是直接用的visual_memory_bank，跟前面forward不太一样，那我推理也要调整吗？调整的顺序是怎样呢
                    # 给prompt使用bert编码
                    # [batchsize, sequence_length, C] batchsize能保持一致吗？
                    # 方案1
                    prompt_embeds = self.Qformer.bert.embeddings(text_Qformer.input_ids)
