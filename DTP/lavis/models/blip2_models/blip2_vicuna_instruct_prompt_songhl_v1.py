...
@registry.register_model("blip2_vicuna_instruct_prompt_v1_malmm")
    ...
    def forward(self, samples):
        ...
        if self.qformer_text_input:  # 检查是否用QFormer处理文本输入
            ...
            if self.use_memory_bank:
                ...
                Qformer_atts = torch.cat([query_atts, text_Qformer_output.attention_mask], dim=1)

                # print(text_Qformer_output.input_ids)
                # print(text_Qformer_output.input_ids.size())
                ...
                for t in range(T):
                    ...
                    # batchsize能保持一致吗？ 可以
                    # 方案1
                    prompt_embeds = self.Qformer.bert.embeddings(text_Qformer_output.input_ids)
                    # 方案2
