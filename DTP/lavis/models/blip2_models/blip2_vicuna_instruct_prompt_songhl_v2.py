...
class CrossAttention(nn.Module):
    def __init__(self, embed_size, num_heads, ff_hidden_size, dropout=0.1):
        ...
        self.ffn = nn.Sequential(
            nn.Linear(embed_size, ff_hidden_size), #这里报错设备初始化有问题
            nn.GELU(),
    ...
    def forward(self, query, key, value, attention_mask=None):
        ...
        # Softmax 和 Dropout
        attention_weights = F.softmax(attention_scores, dim=-1)  # [B, h, T_q, T_k]
        attention_weights = self.dropout(attention_weights)
        # 加权求和得到输出
        attention_output = torch.matmul(attention_weights, V)  # [B, h, T_q, D]
        # 合并多头输出 [B, T_q, D]
        attention_output = attention_output.transpose(1, 2).contiguous().view(B, T_q, self.embed_size)
        # attention经过线型层输出
        attention_output = self.out_fc(attention_output)  # [B, T_q, embed_size]
        # multi-head attention后的残差连接和层归一化
        x = self.norm1(attention_output + query)  # [B, T_q, embed_size]
        assert not torch.isnan(x).any(), "cross attention 归一化后包含 NaN"
        # 输入feed forward
        # 获取输入数据的 device 和 dtype
        device = query.device  # 以 image_embeds 为基准
        dtype = query.dtype  # 使用 image_embeds 的 dtype
        # 确保 ffn 在正确的设备和数据类型下
        self.ffn.to(device=device, dtype=dtype)
        # 前馈神经网络
        ffn_output = self.ffn(x)  # [B, T_q, embed_size]
        assert not torch.isnan(ffn_output).any(), "ffn_output包含 NaN"
        ffn_output = self.dropout_ffn(ffn_output)
        assert not torch.isnan(ffn_output).any(), "ffn_dropout后包含 NaN"
        # feed forward后的残差连接和层归一化
        # 添加第二个残差连接和层归一化
        output = self.norm2(ffn_output + x)  # [B, T_q, embed_size]
        assert not torch.isnan(ffn_output).any(), "ffn 归一化后包含 NaN"
        # 隐藏层
        return output


@registry.register_model("blip2_vicuna_instruct_prompt_v2_malmm")
class Blip2VicunaInstruct_MALMM(Blip2Base):
    ...
    def __init__(
        ...
        self.cross_attention = CrossAttention(
            embed_size=self.visual_hidden_size,
            num_heads=8,
            ff_hidden_size= self.visual_hidden_size,  # hidden_size设成1.5*embeds_size
            dropout=0.1,
        )
        # self.nn_cross_attention = nn.MultiheadAttention(embed_dim=self.visual_hidden_size, num_heads=8)
        # self.proj = nn.Linear(1408, 768)
